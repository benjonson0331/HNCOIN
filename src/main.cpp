...
#if defined(NDEBUG)
# error "Hanacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe6c07a81f9400b9f0740f249f7f8b3769377d72a93ee3b6778eaf41709a7548f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hanacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hanacoin Signed Message:\n";
...
{
    // Hanacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Hanacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Hanacoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Hanacoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hanacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbc0abc6698f277afccc3e9fb91c128ac71c52105cf10064b14ecd77c889ee4f7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "My first Hana Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047c235eb1ca02b5f55af5a3dd137397199b5192215ae0ffe327584e25613596ceaf98429c499e5aa2b0df8a861cb4013eb306e220305400e0462815bfaa32f0de") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1560928667;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084756514;

        if (fTestNet)
        {
            block.nTime    = 1560928667;
            block.nNonce   = 385464960;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
	{
        ...
        assert(block.hashMerkleRoot == uint256("0xe6c68ead50755cbc5254f14222cfd7597cddc2987bbca9e153a4d66345c44bcd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Hanacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HanacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HanacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HanacoinMiner(CWallet *pwallet)
{
    printf("HanacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hanacoin-miner");
    ...
    try { loop {
        ...
        printf("Running HanacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HanacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HanacoinMiner, pwallet));
}
